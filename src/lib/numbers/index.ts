...
function fiatFormat(val: Numberish, { abbreviated = true, forceThreeDecimals = false }: FormatOpts = {}): string {
  ...
  const format = abbreviated ? FIAT_FORMAT_A : isMoreThanOrEqualToAmount(val, FIAT_CENTS_THRESHOLD) ? FIAT_FORMAT_WITHOUT_DECIMALS : FIAT_FORMAT
...
type NumberFormat = 'integer' | 'fiat' | 'token' | 'apr' | 'feePercent' | 'weight' | 'priceImpact' | 'percentage' | 'slippage' | 'sharePercent' | 'stakedPercentage' | 'boost' | 'gyroPrice'
...
export const isValidNumber = (value: string | number | undefined | null): boolean => isNumber(toNumber(value)) && !isNaN(toNumber(value))
...
export const isGreaterThanZeroValidation = (value: string): string | true => (isValidNumber(value) && !isZero(value) ? true : 'Amount must be greater than 0')
